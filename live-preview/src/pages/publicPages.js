import { list, listPublicEvents, listPublicContent, listMemberContent, listPublicEventMediaAssets, getOne } from "../firebase/dataService.js?v=511";
...
export async function archivePage() {
  ...
  const archiveEvents = await hydrateArchiveEventImages(events, mediaAssets);
  ...
  const items = archiveEvents.length
    ...
    : retrospectives.map((item) => archiveListEditorial(item, sponsors, archiveEvents, mediaAssets, galleries)).join("");
