...
export async function uploadEventMedia(eventId = "", files, metadata = {}, onProgress = () => {}) {
  ...
  if (!firebase) throw new Error("Firebase Storage ist nicht erreichbar. Es wurde kein lokales Ersatzmedium gespeichert.");
  const results = [];
...
export async function uploadEntityImage(collection, entityId, file) {
  ...
  if (!firebase) throw new Error("Firebase Storage ist nicht erreichbar. Es wurde kein lokales Ersatzbild gespeichert.");
...
export async function uploadGalleryImages(galleryId, files, onProgress = () => {}) {
  const firebase = await getFirebaseServices();
  if (!firebase) throw new Error("Firebase Storage ist nicht erreichbar. Es wurde keine lokale Ersatzgalerie gespeichert.");
  const results = [];
  for (let index = 0; index < files.length; index += 1) {
    const file = files[index];
