...
export async function listPublicContent(collectionName) {
  ...
  if (collectionName === "members") {
    const [queried, allMembers] = await Promise.all([
      cachedConstrainedList(collectionName, filters.members).catch(() => []),
      list("members").catch(() => [])
    ]);
    return Array.from(new Map([...queried, ...allMembers].map((member) => [member.id, member])).values()).filter(isPublicLiveMember);
  }
