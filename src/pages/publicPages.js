...
function archiveEditorialArticle(item, partners = []) {
  ...
  return `<article class="archive-article">
    ...
    <div class="archive-article__body">
      ...
      <a class="link" href="#/retrospective/${item.id}">Rueckblick lesen ‚Üí</a>
    </div>
  </article>`;
}
...
function internalIcon(name = "") {
  ...
  return `<span class="internal-card__icon" aria-hidden="true">${escapeHtml(labels[name] || "‚Ä¢")}</span>`;
}
...
function aboutStickyContent(events = [], board = [], members = [], topics = []) {
  ...
  return `<aside class="internal-about-sticky" aria-label="Aktuelle Inhalte">
    <section class="internal-sticky-section">
      <a class="internal-sticky-section-title" href="#/events">Aktuelles Event <span aria-hidden="true">‚Üí</span></a>
      ${rubricRotator(eventSlides, renderEvent, `<a class="internal-sticky-event" href="#/events"><span class="internal-sticky-event__picto">${aboutPicto("breakfast")}</span><span><strong>Neue Termine in Vorbereitung</strong><small>Zur Event√ºbersicht</small></span></a>`)}
    </section>
    <section class="internal-sticky-section">
      <a class="internal-sticky-section-title" href="#/topics">Themen <span aria-hidden="true">‚Üí</span></a>
    ...
    <section class="internal-sticky-section">
      <a class="internal-sticky-section-title" href="#/board">Vorstand <span aria-hidden="true">‚Üí</span></a>
      ${rubricRotator(boardSlides, renderBoardPair, `<a class="internal-sticky-person" href="#/board">Vorstand ansehen</a>`)}
    </section>
    <section class="internal-sticky-section">
      <a class="internal-sticky-section-title" href="#/members">Mitglieder <span aria-hidden="true">‚Üí</span></a>
...
function internalDesktopSection(block, meta) {
  ...
  const cta = block.button_text ? `<a class="button button--primary button--small" href="${escapeHtml(block.button_ziel || detailHref)}">${escapeHtml(block.button_text)}</a>` : `<a class="link" href="${detailHref}">Mehr lesen ‚Üí</a>`;
...
export async function internalDetailPage(bereich, slug) {
  ...
  return publicShell(meta.active, `${subhero(meta.eyebrow, block.titel, block.kurztext)}
    <section class="section"><div class="container internal-detail"><a class="link" href="#/${meta.route}">‚Üê Zurueck</a><article class="detail-main"><div class="editorial-text">${articleParagraphs(block.langtext)}</div>${cta}</article></div></section>`);
}
...
function cleanNewsDetailText(text = "", title = "", subtitle = "", slug = "") {
  ...
  const titleKey = String(title || "").toLowerCase().replace(/[^a-z0-9‰ˆ¸ﬂ]+/gi, " ").trim();
  const firstKey = first.toLowerCase().replace(/[^a-z0-9‰ˆ¸ﬂ]+/gi, " ").trim();
  ...
  return publicShell("events", `${subhero(event.eventType, event.title, event.subtitle)}
    <section class="section event-detail-section"><div class="container detail-grid event-detail-grid">
      <article class="detail-main">
        ...
        ${haseparateLongText ? `<h2>R√ºckblick</h2><div class="editorial-text">${articleParagraphs(longText)}</div>` : ""}
...
export async function newsPage(query = new URLSearchParams()) {
  ...
  return publicShell("news", `${subhero("News", "Aktuelles von PROdigitalTV.", "Meldungen, Hinweise und Neuigkeiten aus dem Verein und der digitalen Medienwirtschaft.")}
    <section class="section"><div class="container">${news.length ? `
      ...
      ${!filteredNews.length ? `<div class="alert">Zu dieser Rubrik sind aktuell keine News verˆffentlicht.</div>` : ""}
    ` : `<div class="alert">Aktuell sind keine News verˆffentlicht.</div>`}</div></section>`);
}
  ...
  return publicShell(detailSection, `${subhero("", detailTitle, detailIntro)}
    <section class="section"><div class="container detail-grid">
      ...
      <aside class="detail-aside">
        ...
        <div class="fact"><label>Rubrik</label><strong>${escapeHtml(item.isRetrospective ? "R√ºckblick" : item.category || "News")}</strong></div>
...
function memberEventContactsFields(member = {}) {
  ...
  return `<div class="form-card form-grid member-event-contacts">
    <div>
      ...
      <p class="muted">${limit === 1 ? "Einzelmitglieder kˆnnen eine eventberechtigte Person hinterlegen." : "Firmenmitglieder kˆnnen bis zu 5 eventberechtigte Personen hinterlegen."}</p>
    </div>
...
function memberProfileForm(member = {}, user = {}, options = {}) {
  ...
  return `<form id="member-profile-form" class="form-card form-grid" data-member-id="${escapeHtml(member.id)}">
    ...
    <p class="muted">${adminMode ? "Als Admin kˆnnen Sie den ausgew‰hlten Mitgliedsdatensatz bearbeiten." : "Diese Angaben werden direkt im eigenen Mitglieder-Datensatz gespeichert."}</p>
    ...
    <div class="form-grid--two">
      <div class="field"><label>Straﬂe</label><input name="street" value="${escapeHtml(member.street || "")}"></div>
